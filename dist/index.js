...
const {
  ...
  GITHUB_TOKEN,
  GITHUB_WORKSPACE
} = process.env; // eslint-disable-next-line import/no-dynamic-require
...
async function createCheck() {
  ...
  const {
    data
  } = await request(`https://api.github.com/repos/${owner}/${repo}/check-runs`, {
    method: 'POST',
    headers,
    body
  });
...
function eslint() {
  ...
  for (const result of results) {
    ...
    const path = filePath.substring(GITHUB_WORKSPACE.length + 1); // eslint-disable-next-line no-restricted-syntax
